...
export default function QNAPage() {
  ...
  const fetchPosts = async () => {
    try {
      if (isSupabaseConfigured()) {
        ...
        // Mock 데이터 사용
        setPosts(mockPosts.filter((p) => p.category === '질문 & 답변'));
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-raycast-white">
      ...
      />
      <Script id="ezoic-init-qna" strategy="afterInteractive">
        {`
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-gray-600">
              소프트웨어 사용법, 하드웨어 연결, 문제 해결 등에 대한 질문을 올려주세요.
            </p>
          </div>
          ...
          <div className="mb-6 flex flex-col sm:flex-row gap-4 items-center justify-between">
            <input
              ...
              className="flex-1 max-w-md px-4 py-2 border border-gray-300 rounded-button focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <button
              onClick={() => {
                if (!user) {
                ...
                router.push('/community/write?category=질문 & 답변');
              }}
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-500">
              {searchQuery ? '검색 결과가 없습니다.' : '아직 질문이 없습니다. 첫 질문을 올려보세요!'}
            </div>
          ) : (
